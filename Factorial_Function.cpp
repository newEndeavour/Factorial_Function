/*
  ...
  Editions:	Please go to Factorial_Function.h for Edition Notes.

  Credits:	https://en.wikipedia.org/wiki/Factorial
		http://www.mymathlib.com/functions/combinatorial.html

  Factorial_Function.cpp - Library for 'duino
...
static const int Nf = sizeof(factorials) / sizeof(long double);
...
static const int Nlnf = sizeof(ln_factorials) / sizeof(long double);
...
// Bernoulli numbers B(2),B(4),B(6),...,B(20)  
// Only B(2),...,B(10) currently used.
...
// Active /////////////////////////////////////////////////////////////////
// Factorial
// This function computes n! for 0 <= n <= Factorial_Max_Arg().  
// If n > Factorial_Max_Arg(), then DBL_MAX is returned and if n < 0, then
// 0 is returned.
double Factorial(int n) 
{
	...
	if (n<0) 
        ...
        // For a large positive argument (n>=Nf) return DBL_MAX //
	if (n>=Nf) 
		return DBL_MAX;
...
long double xFactorial(int n) 
{
	...
	// For a large postive argument (n >= Nf) return DBL_MAX //
   	if (n>=Nf) 
...
// Ln_Factorial
// returns ln(n!).
// If n < 0, then -DBL_MAX is returned
double Ln_Factorial(int n) 
{
	...
	// If n<Nlnf then return ln(n!)
	if (n<Nlnf) 
		return (double) ln_factorials[n];
...
// xLn_Factorial_Asymptotic_Expansion
// function estimates log(n!) by evaluating the asymptotic expression:
//         ln(n!) ~ ln(2sqrt(2pi)) - (n+1) + (n+1/2) ln (n+1)             
//                    Sum B[2j] / [ 2j * (2j-1) * (n+1)^(2j-1) ], 
//	summed over j from 1 to 6 and where B[2j] is the 2j-th Bernoulli number
static long double xLn_Factorial_Asymptotic_Expansion(int n) 
{
const int  m = 6;
long double term[6];
